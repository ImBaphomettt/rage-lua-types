
--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see SC_INBOX_GET_TOTAL_NUM_MESSAGES
--@usage undefined SC_INBOX_GET_TOTAL_NUM_MESSAGES();

--@return undefined
function ScInboxGetTotalNumMessages() end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see _SC_INBOX_MESSAGE_INIT
--@usage undefined _SC_INBOX_MESSAGE_INIT(int p0);
--@params p0 int
--@return undefined
function ScInboxMessageInit(p0) end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see _IS_SC_INBOX_VALID
--@usage undefined _IS_SC_INBOX_VALID(int p0);
--@params p0 int
--@return undefined
function IsScInboxValid(p0) end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see SC_INBOX_MESSAGE_GET_DATA_INT
--@usage undefined SC_INBOX_MESSAGE_GET_DATA_INT(int p0,const char* context,int* out);
--@params p0 int
--@params context const char*
--@params out int*
--@return undefined
function ScInboxMessageGetDataInt(p0,context,out) end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see _SC_INBOX_MESSAGE_POP
--@usage undefined _SC_INBOX_MESSAGE_POP(int p0);
--@params p0 int
--@return undefined
function ScInboxMessagePop(p0) end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see SC_INBOX_MESSAGE_GET_DATA_STRING
--@usage undefined SC_INBOX_MESSAGE_GET_DATA_STRING(int p0,const char* context,char* out);
--@params p0 int
--@params context const char*
--@params out char*
--@return undefined
function ScInboxMessageGetDataString(p0,context,out) end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see SC_PRESENCE_ATTR_SET_FLOAT
--@usage undefined SC_PRESENCE_ATTR_SET_FLOAT(Hash attrHash,float value);
--@params attrHash Hash
--@params value float
--@return undefined
function ScPresenceAttrSetFloat(attrHash,value) end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see SC_PRESENCE_ATTR_SET_INT_EX
--@usage undefined SC_PRESENCE_ATTR_SET_INT_EX(const char* attrName,int value,BOOL p2);
--@params attrName const char*
--@params value int
--@params p2 BOOL
--@return undefined
function ScPresenceAttrSetIntEx(attrName,value,p2) end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see SC_PRESENCE_ATTR_SET_FLOAT_EX
--@usage undefined SC_PRESENCE_ATTR_SET_FLOAT_EX(const char* attrName,float value,BOOL p2);
--@params attrName const char*
--@params value float
--@params p2 BOOL
--@return undefined
function ScPresenceAttrSetFloatEx(attrName,value,p2) end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see SC_PROFANITY_CHECK_STRING
--@usage undefined SC_PROFANITY_CHECK_STRING(const char* string,int* token);
--@params string const char*
--@params token int*
--@return undefined
function ScProfanityCheckString(string,token) end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see SC_PRESENCE_ATTR_SET_STRING_EX
--@usage undefined SC_PRESENCE_ATTR_SET_STRING_EX(const char* attrName,const char* value,BOOL p2);
--@params attrName const char*
--@params value const char*
--@params p2 BOOL
--@return undefined
function ScPresenceAttrSetStringEx(attrName,value,p2) end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see _SC_INBOX_MESSAGE_GET_STRING
--@usage undefined _SC_INBOX_MESSAGE_GET_STRING(int p0);
--@params p0 int
--@return undefined
function ScInboxMessageGetString(p0) end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see SC_PROFANITY_GET_CHECK_IS_PENDING
--@usage undefined SC_PROFANITY_GET_CHECK_IS_PENDING(int token);
--@params token int
--@return undefined
function ScProfanityGetCheckIsPending(token) end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see SC_PROFANITY_GET_STRING_PASSED
--@usage undefined SC_PROFANITY_GET_STRING_PASSED(int token);
--@params token int
--@return undefined
function ScProfanityGetStringPassed(token) end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see SC_PROFANITY_GET_CHECK_IS_VALID
--@usage undefined SC_PROFANITY_GET_CHECK_IS_VALID(int token);
--@params token int
--@return undefined
function ScProfanityGetCheckIsValid(token) end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see _0xD635DF6BAA5A6017
--@usage undefined _0xD635DF6BAA5A6017();

--@return undefined
function N_0xd635df6baa5a6017() end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see SC_PROFANITY_GET_STRING_STATUS
--@usage undefined SC_PROFANITY_GET_STRING_STATUS(int token);
--@params token int
--@return undefined
function ScProfanityGetStringStatus(token) end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see _0x060BBAD634C2B44B
--@usage undefined _0x060BBAD634C2B44B();

--@return undefined
function N_0x060bbad634c2b44b() end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see _0x9F6DCD0C939C71E9
--@usage undefined _0x9F6DCD0C939C71E9();

--@return undefined
function N_0x9f6dcd0c939c71e9() end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see _0xB4411D4D6B81438E
--@usage undefined _0xB4411D4D6B81438E();

--@return undefined
function N_0xb4411d4d6b81438e() end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see _0x89D9BDE7334B110F
--@usage undefined _0x89D9BDE7334B110F();

--@return undefined
function N_0x89d9bde7334b110f() end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see _0x09937EB0CEBC2F9F
--@usage undefined _0x09937EB0CEBC2F9F();

--@return undefined
function N_0x09937eb0cebc2f9f() end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see _0xCBF743C984695CF3
--@usage undefined _0xCBF743C984695CF3();

--@return undefined
function N_0xcbf743c984695cf3() end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see _0x03C03ABBABBEF752
--@usage undefined _0x03C03ABBABBEF752();

--@return undefined
function N_0x03c03abbabbef752() end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see _0x1BDB56DB258F052D
--@usage undefined _0x1BDB56DB258F052D();

--@return undefined
function N_0x1bdb56db258f052d() end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see _0xC8FC3B2432E8229D
--@usage undefined _0xC8FC3B2432E8229D();

--@return undefined
function N_0xc8fc3b2432e8229d() end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see _0x85EA0BEC7B1F7622
--@usage undefined _0x85EA0BEC7B1F7622();

--@return undefined
function N_0x85ea0bec7b1f7622() end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see _0x7C981DE05A7403A0
--@usage undefined _0x7C981DE05A7403A0();

--@return undefined
function N_0x7c981de05a7403a0() end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see _0x3519CC3525319A96
--@usage undefined _0x3519CC3525319A96();

--@return undefined
function N_0x3519cc3525319a96() end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see _0x62B384FEFDE06817
--@usage undefined _0x62B384FEFDE06817();

--@return undefined
function N_0x62b384fefde06817() end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see _0x049D2196D9D11184
--@usage undefined _0x049D2196D9D11184();

--@return undefined
function N_0x049d2196d9d11184() end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see _0x91C9E2A0F9DD6DD4
--@usage undefined _0x91C9E2A0F9DD6DD4();

--@return undefined
function N_0x91c9e2a0f9dd6dd4() end

--@description This natives does not have an official description.
--@module NATIVE
--@submodule SOCIALCLUB
--@see _0x11EA52CAD1B55910
--@usage undefined _0x11EA52CAD1B55910();

--@return undefined
function N_0x11ea52cad1b55910() end
